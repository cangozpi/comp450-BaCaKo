...
class Env:
    ...
    def getReward(self, oldBudgetState, newBudgetState, date, action, type):

        intrinsicReward = 0
        if type == 2:
            intrinsicReward = self.getInstrinsicReward(date, newBudgetState, action, type)
        elif type == 3:
            intrinsicReward = self.getInstrinsicReward(date, newBudgetState, action, type)

        return newBudgetState[0] - oldBudgetState[0] + newBudgetState[1] - oldBudgetState[1] + intrinsicReward
    ...
    def getInstrinsicReward(self, date, budget,action, type):
        if type == 2: #type = 2 short term model comparison with long term model
            day = date[0]
            time = date[1]

            hour = time //100

            dateAndTime = (day, hour * 100)

            df = self.stateSpaceLtDict.get(dateAndTime)
            arrayVersion = []
...
# before 4089 lt validation , 4089 sonrası validation 20210504 saat 10
    ...
    def step(self, state, action, type):
        ...
        reward = self.getReward(state[-2:], nextState[-2:],state[0], action, type)
